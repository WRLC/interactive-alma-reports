...
<head>
    <?php $page_title = 'Loans by Patron by University';
...
<body>
    ...
    function fetchXmlData(string $startDate, string $endDate, string $location, string $apikeyinteractive): SimpleXMLElement
    {
        ...
        $path = '%2Fshared%2FWashington%20Research%20Library%20Consortium%20(WRLC)%20Network%2FReports%2FAPI%2FAPI%20-%20rpt_classYtdPatr';
...
echo "<p>Number of <Row> items: " . $rowCount."</p>";
  echo '</div>
    ...
    function generateCSV(SimpleXMLElement $xmlData): string
    {
        ...
        // Add the CSV headers
        fputcsv($file, ['Lender Institution', 'Collection', 'Date', 'Total']);

        // Add data rows
        foreach ($xmlData->QueryResult->ResultXml->rowset->Row as $row) {
            fputcsv($file, [
                (string)$row->Column1, // Lender Institution
                (string)$row->Column2, // Collection
                (string)$row->Column3, // Date
                (float)$row->Column6  // Total
            ]);
        }
    ...
    <div class="container">
        ...
        <?php if ($xmlData) : ?>
            <div class="row justify-content-center mt-5">
                <div class="col-lg-8">
                    <h2 class="text-center">Loans by Patron by University :
                    ...
                    <table class="table table-striped table-bordered">
                        ...
                        <tbody>
                            ...
                            foreach ($xmlData->QueryResult->ResultXml->rowset->Row as $row) :
                                $column6Value = (float)$row->Column6; // Convert Column6 value to float for summation
                                $totalSum += $column6Value; // Sum up Column4 values
                                ?>
                                <tr>
                                    <td><?php echo (string)$row->Column1; ?></td>
                                    <td><?php echo (string)$row->Column2; ?></td>
                                    <td><?php echo (string)$row->Column3; ?></td>
                                    <td><?php echo $column6Value; ?></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
