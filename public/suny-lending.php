...
function fetchXmlData(string $startDate, string $endDate, string $apikeyinteractive): ?SimpleXMLElement
{
    $url = 'https://api-na.hosted.exlibrisgroup.com/almaws/v1/analytics/reports?'
         . 'path=%2Fshared%2FWashington%20Research%20Library%20Consortium%20(WRLC)%20Network%2FReports%2FAPI%2FAPI%20-%20rpt_TotalLendingSUNY'
         . '&limit=1000'
         . '&col_names=false'
...
if ($xmlData && isset($xmlData->QueryResult->ResultXml->rowset)) {
    foreach ($xmlData->QueryResult->ResultXml->rowset as $rowset) {
        if (isset($rowset->Row)) {
            foreach ($rowset->Row as $row) {
                $c1 = (string)$row->Column1;
                $reqType = (string)$row->Column2;
...
$page_title = 'SUNY - Lending Requests';
?>
<!DOCTYPE html>
<html lang="en">
<head>
